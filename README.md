...
├── requirements.txt
├── README.md
├── 00_instructions/           # How to download the raw Kaggle dataset
├── Paper/                     # Reference paper (Kennedy et al., 2022)
├── 01_data_raw/                # Raw, unzipped Kaggle dataset (not committed)
│   └── archive/                # "Vehicles Image Dataset" from Kaggle
...
- Sourced from the Kaggle dataset [Vehicles Image
  Dataset](https://www.kaggle.com/datasets/mmohaiminulislam/vehicles-image-dataset)
  (see `00_instructions/` for download steps); only the `bicycle` and
  `motorcycle` classes are extracted, de-duplicated by file hash, and
  re-encoded from `.webp` to `.jpg` where needed.
...
# Kaggle dataset into 01_data_raw/archive/.
